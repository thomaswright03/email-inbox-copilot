...
- **Today's Summary tab** — reads the mail you received today that is still in your inbox (up to the newest 100, and it says so when there were more; see [Which mail counts](#which-mail-counts)) and asks Gemini to sort it into an **Actionable Briefing**: **Needs a reply**, **FYI**, and **Noise** (collapsed), plus **Has a deadline** only when the deployment sets `AI_DEADLINE_DETECTION=1` (off by default; never for law-firm or other regulated mailboxes). Each item shows the sender, a one-line "what they want", with deadline detection on any date the email names ("Due today: 5 pm" when that date is today in your time zone, always labelled "AI guess, check the email"), and **Open in Gmail**, **Snooze**, **Remind me** and **Done**:
  - **Done** archives the message in Gmail and can be undone from the toast.
  - **Snooze** (later today, tomorrow morning, next week) hides the item until then, and it comes back highlighted. Gmail's API has no snooze, so this happens in Inbox Buddy only: nothing changes in Gmail, the browser keeps just the Gmail message and thread ids and the time (localStorage, per account), and the server only records that Snooze was used, not which message. Snoozed items are listed under **Snoozed** with **Show now**.
  - **Remind me** keeps the item where it is and, at the chosen time, shows a browser notification if you allowed them and highlights the item (or, for an email from an earlier day, shows a banner that opens it in Gmail). **Reminders only fire while an Inbox Buddy tab is open in that same browser**: nothing is sent from a server, so with the tab closed (or in another browser or device) you get the reminder the next time you open Inbox Buddy there. The notification is generic ("Inbox Buddy reminder") and never shows the sender or subject. The button's hint and the "Reminder set" message say so too.

  The header counts what is left, in place of "N messages today": "3 need a reply · 1 deadline today · 14 FYI"; with only Noise left it says how many are collapsed there ("Nothing needs action outside Noise · 5 in Noise, check them") rather than an all-clear. With deadline detection on, the model is told each email's Date header and your local date, time and time zone, and returns each date it finds both as written ("Fri") and as a calendar date, which is how "deadline today" is worked out. Its answer is structured JSON checked item by item against a schema (`lib/ai.ts` `parseTriage`), shown as plain text only. The same model calls (the *triage*) also give every email a spam verdict for the Spam Flashcards tab. The inbox is triaged in chunks of 25 emails, one call each, made at the same time, so a dashboard load costs one Gemini call per 25 emails (at most 4 for 100), and none when nothing new has arrived since the last one (Done, Undo, Delete and Refresh reuse it). If one chunk's call fails, just those emails are sorted by simple rules; a failure is remembered for a minute so reloading doesn't retry it straight away. With AI features off, or when every call fails, it lists the messages to check and the likely bulk mail using simple rules instead, and says so; while AI is off on the deployment the tab is called **Today's Mail**, because a rule-sorted list is not a summary. It shows when it was last updated, has a Refresh button, and links every message to Gmail.
