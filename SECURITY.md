...
| Data left behind | Audit rows carry the Google account id, never the email, and are deleted after `AUDIT_RETENTION_DAYS` (default 90); expired cache rows are deleted on write; sign-out purges the user's cache rows in every layer; cache keys hold the Google account id, not the email address | `lib/db-cache.ts`, `auth.ts` (`events.signOut`) |
