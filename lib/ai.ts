...
export async function summarizeToday(emails: ParsedEmail[]): Promise<string> {
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Here are today's emails. Write a concise, skimmable summary (grouped by importance, use short bullet points) of what actually matters. Ignore obvious marketing/newsletter noise unless something is time-sensitive. Do not include a preamble.\n\n${digest}`,
      },
    ],
  });
