...
export async function readInbox(accessToken: string, userId: string, day: LocalDay, context: string): Promise<InboxRead> {
  try {
    return await getOrSetCached(userCacheKey("messages", userId, day), INBOX_CACHE_TTL_MS, async () => ({
      ...(await fetchRecentMessages(accessToken, day.startSeconds)),
...
export async function maybeRefresh(req: Request, userId: string, day: LocalDay, kind: "today" | "spam"): Promise<void> {
  if (new URL(req.url).searchParams.get("refresh") !== "1") return;
  const cached = await peekCached<InboxRead>(userCacheKey("messages", userId, day));
